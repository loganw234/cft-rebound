...
int main(void){
    ...
    /* max_iter is left alone, which now means the default for the
     * format - 60 at binary256, rather than REBOUND's 12. It used to
     * mean 12 at every format, and that was a real gap against this
     * repository's own intent, but on this problem it was not costing
     * anything: measured, a cap of 12 was exceeded on 2 step attempts
     * out of 180 and the binary64 output is bit-identical either way.
     * Set s->max_iter yourself, or call cft_ias15_configure(), if you
     * want a specific cap. */
