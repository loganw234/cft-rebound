...
static void say_adaptive_mode(const struct cft_support_ctx *c, char *b, size_t n){
    snprintf(b, n, "adaptive_mode %d is not implemented. PRS23 (2), REBOUND's default "
                   "since January 2024, and AARSETH85 (3) are; INDIVIDUAL (0) and "
                   "GLOBAL (1) take REBOUND's other error estimate entirely and are "
                   "not.", c->adaptive_mode);
}
