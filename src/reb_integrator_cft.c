...
const struct reb_integrator cft_ias15_integrator = {
    .documentation =
        ...
        "gravity routine, r->map, variational particles and MEGNO are refused "
        "rather than approximated; the exact list is cft_support_rows in "
        "src/cft_supported.c, and every entry names itself when it refuses.\n\n"
        "Test particles are supported: set r->N_active to the number of massive "
        "bodies, with r->testparticle_type, and only an N_active greater than "
        "r->N is refused - REBOUND's own gravity loop reads past the end of "
        "r->particles there, so there is no behaviour to reproduce.\n\n"
